-- Migration: Fix Subscription Sync - Real-time Premium Status Updates (Corrected)
-- Created: 2025-02-02
-- Description: Adds triggers and functions for automatic subscription sync (with function drop)

-- 1. Drop existing function if it exists (to avoid parameter name conflicts)
DROP FUNCTION IF EXISTS update_user_premium_features(UUID);

-- 2. Create enhanced function to update user premium features
...
-- 3. Drop existing trigger function if it exists
DROP FUNCTION IF EXISTS trigger_update_user_premium_features();

-- 4. Create trigger function for automatic subscription updates
...
-- 5. Create trigger on subscriptions table (drop first if exists)
...
-- 6. Drop existing helper functions if they exist
DROP FUNCTION IF EXISTS sync_all_user_subscriptions();
DROP FUNCTION IF EXISTS get_user_subscription_status(UUID);

-- 7. Function to manually sync all user subscriptions (for admin use)
...
-- 8. Function to get user's current subscription status with premium features
...
-- 10. Run initial sync for all existing users
...
DO $$
BEGIN
    RAISE NOTICE 'Subscription sync migration completed successfully (corrected version)';
    RAISE NOTICE 'All existing functions have been dropped and recreated';
